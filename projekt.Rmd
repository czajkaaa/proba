...
p1 <- ggplot(completed_data, aes(x = price)) +
  geom_density(fill = "#69b3a2", alpha = 0.5) +
  stat_function(fun = dnorm, args = list(mean = mean(completed_data$price), sd = sd(completed_data$price)), 
                color = "red", size = 1) +
  labs(title = "Rozkład cen (Gęstość vs Krzywa Normalna)", x = "Cena", y = "Gęstość") +
  theme_minimal()

# Wykres Q-Q
p2 <- ggqqplot(completed_data$price, title = "Wykres Q-Q dla ceny")
...
```{r echo=FALSE, message=FALSE, warning=FALSE, fig.width=10, fig.height=5}
test_kw <- kruskal.test(price ~ type, data = completed_data)
test_kw
```
**Interpretacja** 
...
# Budowa modelu
model <- lm(price ~ squareMeters + rooms + centreDistance + buildYear + hasElevator, data = completed_data)
...
```
**Wnioski z modelu**
Każdy dodatkowy metr kwadratowy powierzchni zwiększa cenę średnio o r round(model_results$estimate[2], 2) PLN.Każdy kilometr od centrum obniża cenę średnio o r abs(round(model_results$estimate[4], 2)) PLN.Wszystkie wybrane zmienne są istotne statystycznie ($p < 0.05$).